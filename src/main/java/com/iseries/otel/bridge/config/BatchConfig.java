...
public class BatchConfig {
    ...
    public Job logConversionJob(Step logConversionStep) {
        return new JobBuilder("logConversionJob", jobRepository).start(logConversionStep).build();
    }

    @Bean
    public Step logConversionStep(
    ...
    @Bean
    @StepScope
    public MultilineLogItemReader reader(
        ...
        return new MultilineLogItemReader(
                new FileSystemResource(new File(filePath)), effectivePattern);
    }

    @Bean
    @StepScope
    public GrokItemProcessor processor(
            @Value("#{jobParameters['parser.grok.pattern']}") String grokPattern) {
